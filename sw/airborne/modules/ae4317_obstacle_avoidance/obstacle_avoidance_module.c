...
#include "obstacle_avoidance_module.h"
...
/* Static functions */
static void *obstacle_avoidance_thread(void *data);                   //< The main optical flow calculation thread


/**
 * Initialize the optical flow module for the bottom camera
 */
void obstacle_avoidance_module_init(void)
{
 ...
 */
void obstacle_avoidance_module_run(void)
{
...
/**
 * Start the module and the optical flow calculation thread
 */
void obstacle_avoidance_module_start(void)
{
  ...
  int rc = pthread_create(&opticflow_calc_thread, NULL, obstacle_avoidance_thread, NULL);
  if (rc) {
 ...
 * Stop the module
 */
void obstacle_avoidance_module_stop(void)
{
  // Stop the capturing
...
/**
 * The main obstacle avoidance thread
 * This thread calculates total optical flow, determines featureless zone size.
 * Next, it issues navigation commands based on these quantities, when they exceed
 * specified thresholds
 */
#include "errno.h"
static void *obstacle_avoidance_thread(void *data __attribute__((unused))) {
      ...
      if (nav_block == 6) {
      	if (OF_total_av > NAV_TURN_THRESHOLD_OF) {
        	if (OF_left_av > OF_right_av) {
          	// Turn right if more optical flow to the left
          	heading_change = NAV_HEADING_CHANGE;
        	}
        	if (OF_left_av < OF_right_av) {
        	  // Turn left if more optical flow to the right
        	  heading_change = -NAV_HEADING_CHANGE;
        	}
      	}
      	if (OF_fz_av > NAV_TURN_THRESHOLD_FZ) {
        	if (OF_fz_pos_av < 320/2) {
          	// Turn right if featureless zone is to the left
          	heading_change = NAV_HEADING_CHANGE;
        	}
        	if (OF_fz_pos_av > 320/2) {
        	  // Turn left if featureless zone is to the right
        	  heading_change = -NAV_HEADING_CHANGE;
        	}
      	}
